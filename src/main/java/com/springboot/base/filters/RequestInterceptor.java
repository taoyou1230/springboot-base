...
//            response.sendRedirect(contextPath + "/error/500");
            this.sendError(response,"系统错误，500");
            return false;
        }else if(response.getStatus()==404){
//            response.sendRedirect(contextPath + "/error/404");
            this.sendError(response,"请求路径有误或是页面不存在！");
            return false;
        }else if(response.getStatus() == 401){
            this.sendError(response,"用户认证失败！");
            return false;
        }
...
//        System.out.println(">>>MyInterceptor1>>>>>>>在整个请求结束之后被调用，也就是在DispatcherServlet 渲染了对应的视图之后执行（主要是用于进行资源清理工作）");
    ...
    private void sendError(HttpServletResponse response,String msg){
        ...
        try {
            ...
            String error = JSONObject.toJSONString(new ResultObject(false, msg, null));
            writer.write(error);
        }catch (Exception e){
            e.printStackTrace();
        }finally {
